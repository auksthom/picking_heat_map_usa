...
def load_picking_data():
    ...
    df_bp.columns = df_bp.columns.str.strip().str.replace(' ', '_').str.lower()
...
try:
    ...
    # Fallback check: if the column is 'bay' instead of 'bay_name'
    if 'bay_name' not in df_map_bp.columns and 'bay' in df_map_bp.columns:
        df_map_bp = df_map_bp.rename(columns={'bay': 'bay_name'})
    ...
    # --- 6. GRID MAPPING ---
    if df_map_bp.empty:
        st.error("No layout data found.")
    else:
        max_r = int(df_map_bp['grid_row'].max() + 1)
        max_c = int(df_map_bp['grid_col'].max() + 1)
